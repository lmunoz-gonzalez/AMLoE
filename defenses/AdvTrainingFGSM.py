...
def advTrainingIterFGSM(model, optimizer, loss, x, y, epsilon, device):  
    model.to(device)
    x.to(device)
    y.to(device)
    #Reset gradients
    optimizer.zero_grad()
    #Perturbed examples
    x_adv = fgsm.fgsm_attack(x, y, model, epsilon, device, attack_type='untargeted')
    x_adv.to(device)
    pred = model(x_adv)
    pred.to(device)
...
"""
Function for adversarially training the model with FGSM attack
Parameters:
    + model             
    + loss             
    + optimizer         
...
Output:
    ...
    + optimizer
"""
def AdvTrainFGSM(model, loss, optimizer, train_loader, test_loader, 
          epochs, epsilon, device, epoch_evaluation = 0, epoch_save_checkpoint = 0, 
    ...
    for epoch in range(initial_epoch, epochs + 1):
        ...
        for x_tr, y_tr in train_loader:
            error, _ = advTrainingIterFGSM(model, optimizer, loss, 
                             x_tr.to(device), y_tr.to(device), epsilon, device)
