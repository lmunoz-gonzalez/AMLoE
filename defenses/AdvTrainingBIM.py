...
def advTrainingIterBIM(model, optimizer, loss, x, y, epsilon, alpha, steps, 
                       device, lp_norm = 'linf'):   
    model.to(device)
    x.to(device)
    y.to(device)
    ...
    x_adv = bim.bim_attack(x, y, model, epsilon, alpha, steps, 
                       device, attack_type, lp_norm)
    x_adv.to(device)
    pred = model(x_adv)
    pred.to(device)
...
"""
Function for adversarially training the model with BIM attack
Parameters:
    + model             
    + loss             
    + optimizer         
...
def AdvTrainBIM(model, loss, optimizer, train_loader, test_loader, 
          epochs, epsilon, alpha, steps, lp_norm, device, 
    ...
    for epoch in range(initial_epoch, epochs + 1):
        ...
        for x_tr, y_tr in train_loader:
            error, _ = advTrainingIterBIM(model, optimizer, loss, 
                             x_tr.to(device), y_tr.to(device), epsilon, 
                             alpha, steps, device, lp_norm)
